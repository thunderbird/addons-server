...
@validation_task
def validate_file_path(path, hash_, listed=True, is_webextension=False, is_experimental=False, **kw):
    """Run the validator against a file at the given path, and return the
    results.

    Should only be called directly by Validator."""
    if is_webextension:
        return run_addons_linter(path, listed=listed, is_experimental=is_experimental)
...
def run_addons_linter(path, listed=True, is_experimental=False):
